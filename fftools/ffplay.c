...
// 所以每次播放刷新时都重新set_clock一次是最好的做法
// ===================================音频为主时钟同步============================================
// 1.解码后未插入frame队列之前,根据当前帧pts和主时钟同步,如果落后则丢弃,显示前就丢弃了
// 2.在视频刷新时，根据视频时钟和主时钟同步，如果落后则丢弃,丢弃的帧有可能是正在显示但已经到显示结束时间了(其时这里不应该较丢弃，应该叫切换到下一帧）
...
//===================================同步、丢包、丢帧=============================
// 1.丢包（读取packet未插入packet队列之前）:
//      由于用户指定播放时间和播放时长导致，丢弃的是未解码的数据包，包括音频和视频
// 2.丢帧（解码后未插入frame队列之前）: 
//      视频帧pts落后于音频主时钟
// 3.丢帧（从frame队列取出还未播放）: 
//      通常seek操作后，packet队列serial更新了，而从frame队列取出的解码帧的serial还是旧的，说明这帧数据是seek之前的数据，则丢弃
// 4.丢帧（显示时时钟同步）:
//      在视频刷新时，根据视频时钟和主时钟同步，如果落后则丢弃,丢弃的帧有可能是正在显示但已经到显示结束时间了(其时这里不应该较丢弃，应该叫切换到下一帧）
//      ，有可能是未显示的，即后面的帧通过计算也到理论结束时间了
